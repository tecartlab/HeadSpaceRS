...
void orthoCamera::begin(ofRectangle rect, float left, float right, float front, float back, float near1, float far1){
	ofCamera::begin(rect);
	...
	glMatrixMode(GL_PROJECTION);
	glLoadIdentity();
	glOrtho(left * scale, right * scale, front * scale, back * scale, near1 * scale, far1 * scale);
	glMatrixMode(GL_MODELVIEW);
